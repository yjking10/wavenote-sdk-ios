...
final class DemoAudioLibrary {
    ...
    func invalidate() {
        ...
        stopRequested = false; rows = []; collected = []
    ...
    private func downloadNext(_ index: Int) {
        guard proceed() else { return }
        guard index < rows.count else { end(rows.isEmpty ? "设备暂无录音文件" : "同步完成，点击播放本地音频"); return }
        ...
        message = "正在同步 \(index + 1)/\(rows.count)"; changed?()
        let ticket = next()
        cancelDownload = download(file, { [weak self] bytes in
            ...
            if let error { self.rows[index].status = self.isRecording ? "录音开始，下载中断" : error; self.end(self.stopRequested ? "同步已暂停，未完成文件不会用于播放" : error); return }
            guard let path else { self.rows[index].status = "缺少本地文件"; self.end("下载未交付完整文件"); return }
            self.rows[index].localPath = path; self.rows[index].received = file.size; self.rows[index].status = "已下载"
